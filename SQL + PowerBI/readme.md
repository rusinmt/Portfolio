...
ORDER BY [Sales] DESC
```
DIM_SalesTerritory table groups the sum of SalesAmount for each sales territory and orders the results by three specified columns in descending order. This query was used to explore the dataset in SQL Server and did not find a particular use in the model.
```sql
...
For the purpose of using the full territory name wihout repeating,
```dax
